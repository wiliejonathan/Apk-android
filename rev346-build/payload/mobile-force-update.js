(function(){
  ...
  function overlay(){
    ...
    style.textContent=''
      ...
      +'#tf-force-update-btn{width:100%;margin-top:18px;border:0;border-radius:12px;padding:14px 16px;background:#16a34a;color:white;font-weight:800;font-size:14px;cursor:pointer}'
    ...
    btn.addEventListener('click',function(){
      if(!state.latest || !state.latest.url) return;
      btn.disabled=true;
      setStatus('Menyiapkan download update…');
      try{
        window.AndroidUpdate.downloadAndInstall(state.latest.url, state.latest.name || 'TF.Analyzer.Analyst.update.apk');
      }catch(err){
        btn.disabled=false;
        setStatus('Gagal memulai update. Tekan tombol untuk mencoba lagi.');
      }
    });
    return el;
  }
