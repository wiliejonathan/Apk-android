(() => {
    ...
    try {
      const response = await fetch(API_BASE + path, {
        ...
        body: JSON.stringify({ ...(body || {}), deviceType: 'MOBILE', clientType: 'MOBILE',
          mobilePlatform: platformName(), mobileVersion: '1.0.106', remoteRevision: 'REV349',
  ...
  async function startApp() {
    ...
    try {
      ...
      if (root) root.remove();
      startLicenseWatch();
      setTimeout(() => { try { if (typeof window.tfMobileRecoverRenderV31 === 'function') window.tfMobileRecoverRenderV31('token-gate-rev349'); } catch (_) {} }, 400);
    } catch (error) {
      appStarted = false;
  ...
  async function boot() {
    ...
    activeCredentials = null; activeState = null; exposeEphemeralAuth();
    createGate(); bindUi();
    setStatus('Masukkan email dan token untuk aktivasi TF Analyzer.');
  }
